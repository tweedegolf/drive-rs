...
<div
  class={open ? "search filter open" : "search filter"}
  use:clickOutside={{ enabled: open, cb: () => (open = false) }}
>
  <button
    class="filter-box"
    on:click={() =>
      $open_filter === name ? ($open_filter = "") : ($open_filter = name)}
  >
  ...
  <div class={open ? "filter-popup" : "filter-popup hidden"}>
    <div class="filter-list">
      <label>
        <input
          type="checkbox"
          on:click={() => update_sort("alphanumeric")}
          bind:checked={alpha_open}
        /> Alphabetical
      </label>
      <label>
        <input
          type="checkbox"
          on:click={() => update_sort("all_downloads")}
          bind:checked={all_downloads_open}
        /> Downloads (all-time)
      </label>
      <label>
        <input
          type="checkbox"
          on:click={() => update_sort("recent_downloads")}
          bind:checked={recent_downloads_open}
        /> Downloads (recent)
      </label>
      <label>
        <input
          type="checkbox"
          on:click={() => update_sort("newly_added")}
          bind:checked={newly_added_open}
        /> Newly added
      </label>
      <label>
        <input
          type="checkbox"
          on:click={() => update_sort("recently_updated")}
          bind:checked={recently_updated_open}
        /> Recently updated
      </label>
      <label>
        <input
          type="checkbox"
          on:click={() => update_sort("score")}
          bind:checked={text_score}
        /> Text search
      </label>
    </div>
  </div>
