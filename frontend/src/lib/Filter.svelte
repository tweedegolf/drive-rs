<script lang="ts">
  ...
  function sort(
    ...
    let unchecked = Object.entries(items).filter(
    ...
    {#if Object.entries(values).length > 5}
      <div class="filter-bottom">
        ...
        <button
          class={sort_by === NAME
            ? sort_direction
              ? "sort-item sorted asc"
              : "sort-item sorted desc"
            : "sort-item"}
          on:click={() => update_sort(NAME)}
          >Name
        </button>
        <button
          class={sort_by === COUNT
