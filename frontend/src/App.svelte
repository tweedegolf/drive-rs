<script lang="ts">
  ...
  function combine_filters(crate_length: number, selected: any): number[] {
    ...
    for (const filter of selected) {
      ...
      selected_crates = selected_crates.filter((crate) =>
        selected_filter.includes(crate),
      );
    }
  ...
  $: selected_crates = combine_filters(t_crates.length, [
    selected_c,
    selected_d,
    selected_l,
    selected_r,
...
<main>
  <div class="filters">
    ...
    <Filter
      name="Rust Version"
      values={t_indexes.rust_version}
      bind:selected={selected_r}
    />
  </div>
